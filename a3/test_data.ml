...
module DictTester (M:DictionaryMaker) = struct
  ...
  let tests = [
  "name" >:: (fun _ -> assert_equal true (Dict.member 3
  ...
  "rep_ok test" >:: (fun _ -> assert_equal dict (Dict.rep_ok dict));
  "empty_test" >:: (fun _ -> assert_equal true (Dict.is_empty Dict.empty));
  "string_test" >:: (fun _ -> assert_equal true (Dict1.is_empty Dict1.empty));
  "string_test_to_list" >:: (fun _ -> assert_equal ["a";"b";"c";"d";"e";"f";"g"]
    (List.map fst (Dict1.to_list dict1)));
  "size_test" >:: (fun _ -> assert_equal 7 (Dict.size dict));
  "size_test_2"  >:: (fun _ -> assert_equal 6 (Dict.size (Dict.remove 6 dict)));
  "size_test_3"  >:: (fun _ -> assert_equal 7
    (Dict.size (Dict.insert 6 3 dict)));
  "find_test"  >:: (fun _ -> assert_equal (Some 4)  (Dict.find 6 dict));
  "find_test_2" >:: (fun _ -> assert_equal (Some 3)
    (Dict.find 6 (Dict.insert 6 3 dict)));
  "member_test"  >:: (fun _ -> assert_equal true (Dict.member 6 dict));
  "member_test_2"  >:: (fun _ -> assert_equal false
    (Dict.member 6 (Dict.remove 6 dict)));
  "choose_test"  >:: (fun _ -> assert_equal None (Dict.choose Dict.empty));
