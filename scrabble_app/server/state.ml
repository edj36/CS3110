...
(* [get_players] : string list -> player list
...
let rec get_players str=
  let input_string_list = Str.split (Str.regexp " +") (str ^ " ") in
...
let setup str =
  let players = try get_players str with 
  | Error_duplicate_names -> []
  | Error_too_many_players -> []
  | Error_ai_level -> []
  | Failure _ -> [] in 
  begin match players with 
    | [] -> "error"
    | _ -> 
      let src = Yojson.Basic.from_file "info.json" in
